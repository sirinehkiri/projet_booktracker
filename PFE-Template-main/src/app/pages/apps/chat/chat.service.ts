...
export class ChatService {
  ...
  startUnreadPolling(): void {
    ...
    this.pollingSub = interval(3000).subscribe(() => this.fetchUnreadCount());
  }
      ...
      next: (privateMessages: any[]) => {
        this.getGroups().subscribe({
          next: (groups: any[]) => {
            ...
            this.unreadCountSubject.next(privateMessages.length + groupUnread);
            ...
            privateMessages.forEach((msg: any) => {
              ...
              if (senderId != null) {
                ...
                if (senderMap.has(id)) senderMap.get(id).count++;
                else senderMap.set(id, { id, name: senderName, count: 1, type: 'private' });
              }
            });
          ...
          error: (err: any) => console.error('Error fetching groups:', err),
        });
      },
      error: (err: any) => console.error('Error fetching unread messages:', err),
    });
  }
  ...
  markAsRead(otherUserId: number): Observable<any> {
    return this.http.post(`${this.apiUrl}/markAsRead/${otherUserId}`, {}, { headers: this.getHeaders() });
  }
  ...
  createGroup(name: string, memberIds: number[]): Observable<any> {
    return this.http.post(`${this.apiUrl}/groups`, { name, memberIds }, { headers: this.getHeaders() });
  }

  getGroupConversation(groupId: number): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/groups/${groupId}/conversation`, { headers: this.getHeaders() });
  }

  sendGroupMessage(groupId: number, content: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/groups/${groupId}/send`, { content }, { headers: this.getHeaders() });
  }

  markGroupAsRead(groupId: number): Observable<any> {
    return this.http.post(`${this.apiUrl}/groups/${groupId}/markAsRead`, {}, { headers: this.getHeaders() });
  }

  deleteGroupMessage(messageId: number): Observable<any> {
    return this.http.delete(`${this.apiUrl}/groups/delete/${messageId}`, { headers: this.getHeaders() });
  }

  leaveGroup(groupId: number): Observable<any> {
    return this.http.delete(`${this.apiUrl}/groups/${groupId}/leave`, { headers: this.getHeaders() });
  }

  // backend عندك /join
  rejoinGroup(groupId: number): Observable<any> {
    return this.http.post(`${this.apiUrl}/groups/${groupId}/join`, {}, { headers: this.getHeaders() });
  }
